...
# Adjust these pin numbers to your actual wiring!
TRIG_FRONT, ECHO_FRONT = 2, 3   
TRIG_REAR, ECHO_REAR = 4, 14    
...
GPIO.setup([TRIG_FRONT, TRIG_REAR], GPIO.OUT)
GPIO.setup([ECHO_FRONT, ECHO_REAR], GPIO.IN)

# PWM Setup
fl_pwm = GPIO.PWM(FL_EN, 100); rl_pwm = GPIO.PWM(RL_EN, 100)
fr_pwm = GPIO.PWM(FR_EN, 100); rr_pwm = GPIO.PWM(RR_EN, 100)
for pwm in [fl_pwm, rl_pwm, fr_pwm, rr_pwm]: pwm.start(0)

BASE_SPEED = 60
TURN_SPEED = 50

# --- 2. SENSOR & MOTOR FUNCTIONS --- 🕹️

def get_distance(trig, echo):
    """Calculates distance from HC-SR04 in cm."""
...
# --- 4. MAIN CONTROL LOOP --- 🚀
print("AI System Active. Flipped Camera + IR + Obstacle Avoidance.")

try:
    while True:
        # 🛡️ PRIORITY 0: Obstacle Detection (Ahead)
        dist_front = get_distance(TRIG_FRONT, ECHO_FRONT)
        if dist_front < 20: # Stop if anything is closer than 20cm
            print(f"⚠️ OBSTACLE AHEAD: {dist_front}cm")
            stop()
            time.sleep(0.1)
            continue
